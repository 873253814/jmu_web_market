<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="jmu_web.market.book.entity.*"%>
<%@ page import="jmu_web.market.book.dao.*"%>
<%@ page import="jmu_web.market.book.dao.impl.*"%>
<%@ page import="java.util.List"%>
    
<%
    BookDAO dao = new BookDAOImpl();
    List<Book> bookList = dao.getAll();
    String sid = (String)session.getAttribute("sessionId");
    //out.print(sid);
%>
<html>
<head>
    <meta charset="UTF-8">
    <title>达内商城学子餐饮页</title>
    <link href="../css/item.food.css" rel="stylesheet" />
    <link href="../css/header.css" rel="stylesheet" />
    <link href="../css/footer.css" rel="stylesheet" />
    <link href="../css/slide.css" rel="stylesheet" />
</head>
<body>
<!-- 页面顶部-->
<header id="top">
    <div id="logo" class="lf">
        <img src="../img/header/logo.png" alt="logo"/>
    </div>
    <div id="top_input" class="lf">
        <input id="input" type="text" name="content" placeholder="请输入您要搜索的内容"/>
        <a href="search.jsp?title=" class="rt"><img id="search" src="../img/header/search.png" alt="搜索"/></a>
    </div>
    <div class="rt">
        <ul class="lf">
            <li><a href="index.jsp" >首页</a><b>|</b></li>
            <li><a href="collect.jsp" >收藏</a><b>|</b></li>
            <li><a href="order.jsp" >订单</a><b>|</b></li>
            <li><a href="cart.jsp" >购物车</a><b>|</b></li>
            <li><a href="password-change.jsp">设置</a><b>|</b></li>
            <li><a href="login.html">退出</a><b>|</b></li>
            <li><a href="lookforward.html">帮助</a></li>
        </ul>
    </div>
</header>
<!-- nav主导航-->
<div>&nbsp;</div>
<!-- banner部分-->
<div class="ck-slide">
    <ul class="ck-slide-wrapper">
        <li>
            <a href="detail.jsp?isbn=9787121281556"><img src="../img/banner/banner1.png" alt=""></a>
        </li>
        <li style="display:none">
            <a href="detail.jsp?isbn=9787111563891"><img src="../img/banner/banner2.png" alt=""></a>
        </li>
        <li style="display:none">
            <a href="detail.jsp?isbn=9787111564805"><img src="../img/banner/banner3.png" alt=""></a>
        </li>
        <li style="display:none">
            <a href="detail.jsp?isbn=9787115353528"><img src="../img/banner/banner4.png" alt=""></a>
        </li>
        <li style="display:none">
            <a href="detail.jsp?isbn=9787111526285"><img src="../img/banner/banner5.png" alt=""></a>
        </li>
    </ul>
    <a href="javascript:;" class="ctrl-slide ck-prev">上一张</a> <a href="javascript:;" class="ctrl-slide ck-next">下一张</a>
    <div class="ck-slidebox">
        <div class="slideWrap">
            <ul class="dot-wrap">
                <li class="current"><em>1</em></li>
                <li><em>2</em></li>
                <li><em>3</em></li>
                <li><em>4</em></li>
                <li><em>5</em></li>
            </ul>
        </div>
    </div>
</div>
<!-- 特推部分-->

<!--商品-->
<div class="store">
    <div class="store_top">
        <img src="../img/banner/icon.png" alt=""/>
        热搜排行
    </div>

	  <div class="store_content">
        <%for(Book b:bookList){ %>
        <div>
            <a href="detail.jsp?isbn=<%=b.getIsbn()%>"><img src="../img/goods/<%=b.getIsbn() %>/index.jpg" alt=""/></a>
            <p class="one"><%=b.getTitle() %></p>
            <p class="two">
                <span>
                    价格：<span>￥<%=b.getPrice() %></span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span><%=b.getPress() %></span>
                </span>
            </p>
        </div>
       <%} %>
	</div>
</div>

    <!--     <div>
            <a href="detail.html"><img src="../img/goods/9787111526285/index.jpg" alt=""/></a>
            <p class="one">Go程序设计语言（英文版）</p>
            <p class="two">
                <span>
                    价格：<span>￥64.7</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>机械工业出版社 </span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787111532644/index.jpg" alt=""/></a>
            <p class="one">架构即未来</p>
            <p class="two">
                <span>
                    价格：<span>￥81.0</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>机械工业出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787111545682/index.jpg" alt=""/></a>
            <p class="one">Linux运维最佳实践</p>
            <p class="two">
                <span>
                    价格：<span>￥56.5</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>机械工业出版社</span>
                </span>
            </p>
        </div>

	    </div>



		    <div class="store_content">

        <div>
            <a href="detail.html"><img src="../img/goods/9787111563891/index.jpg" alt=""/></a>
            <p class="one">面向机器智能的TensorFlow实践</p>
            <p class="two">
                <span>
                    价格：<span>￥56.5</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>机械工业出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787111564805/index.jpg" alt=""/></a>
            <p class="one">企业IT架构转型之道</p>
            <p class="two">
                <span>
                    价格：<span>￥53.8</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>机械工业出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787115130228/index.jpg" alt=""/></a>
            <p class="one">C Primer Plus</p>
            <p class="two">
                <span>
                    价格：<span>￥49.8</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787115335500/index.jpg" alt=""/></a>
            <p class="one">深入浅出Node.js</p>
            <p class="two">
                <span>
                    价格：<span>￥57.3</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社</span>
                </span>
            </p>
        </div>

	    </div>



		    <div class="store_content">

        <div>
            <a href="detail.html"><img src="../img/goods/9787115353528/index.jpg" alt=""/></a>
            <p class="one">Python基础教程</p>
            <p class="two">
                <span>
                    价格：<span>￥65.6</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787115386397/index.jpg" alt=""/></a>
            <p class="one">Spring MVC学习指南</p>
            <p class="two">
                <span>
                    价格：<span>￥40.7</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社 </span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787115388025/index.jpg" alt=""/></a>
            <p class="one">图解机器学习 </p>
            <p class="two">
                <span>
                    价格：<span>￥40.7</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787115435101/index.jpg" alt=""/></a>
            <p class="one">Python数据科学指南</p>
            <p class="two">
                <span>
                    价格：<span>￥66.2</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>人民邮电出版社</span>
                </span>
            </p>
        </div>

	    </div>



		    <div class="store_content">

        <div>
            <a href="detail.html"><img src="../img/goods/9787121281556/index.jpg" alt=""/></a>
            <p class="one">Hadoop 2.X HDFS源码剖析</p>
            <p class="two">
                <span>
                    价格：<span>￥86.4</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>电子工业出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787121291609/index.jpg" alt=""/></a>
            <p class="one">Go语言学习笔记 </p>
            <p class="two">
                <span>
                    价格：<span>￥73.9</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>电子工业出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787302383666/index.jpg" alt=""/></a>
            <p class="one">MongoDB大数据处理权威指南</p>
            <p class="two">
                <span>
                    价格：<span>￥42.0</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>清华大学出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787302435167/index.jpg" alt=""/></a>
            <p class="one">大数据架构师指南</p>
            <p class="two">
                <span>
                    价格：<span>￥38.3</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>清华大学出版社</span>
                </span>
            </p>
        </div>

	    </div>



		    <div class="store_content">

        <div>
            <a href="detail.html"><img src="../img/goods/9787302459873/index.jpg" alt=""/></a>
            <p class="one">数据挖掘与预测分析</p>
            <p class="two">
                <span>
                    价格：<span>￥84.1</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>清华大学出版社</span>
                </span>
            </p>
        </div>



        <div>
            <a href="detail.html"><img src="../img/goods/9787508344980/index.jpg" alt=""/></a>
            <p class="one">Head First Java（涵盖Java5.0）</p>
            <p class="two">
                <span>
                    价格：<span>￥53.9</span>
                </span>
            </p>
            <p class="three">
                <span>
                出版社：<span>中国电力出版社</span>
                </span>
            </p>
        </div>
 -->

<!-- 品质保障，私人定制等-->
<div id="foot_box">
    <div class="icon1 lf">
        <img src="../img/footer/icon1.png" alt=""/>

        <h3>品质保障</h3>
    </div>
    <div class="icon2 lf">
        <img src="../img/footer/icon2.png" alt=""/>

        <h3>私人定制</h3>
    </div>
    <div class="icon3 lf">
        <img src="../img/footer/icon3.png" alt=""/>

        <h3>学员特供</h3>
    </div>
    <div class="icon4 lf">
        <img src="../img/footer/icon4.png" alt=""/>

        <h3>专属特权</h3>
    </div>
</div>
<!-- 页面底部-->
<div class="foot_bj">
    <div id="foot">
        <div class="lf">
            <p class="footer1"><img src="../img/footer/tedu.png" alt="" class=" footLogo"/></p>
            <p class="footer2"><img src="../img/footer/footerFont.png"alt=""/></p>
            <!-- 页面底部-备案号 #footer -->
            <div class="record">
                2001-2016 版权所有 京ICP证8000853号-56
            </div>
        </div>
        <div class="foot_left lf" >
            <ul>
                <li><a href="#"><h3>买家帮助</h3></a></li>
                <li><a href="#">新手指南</a></li>
                <li><a href="#">服务保障</a></li>
                <li><a href="#">常见问题</a></li>
            </ul>
            <ul>
                <li><a href="#"><h3>商家帮助</h3></a></li>
                <li><a href="#">商家入驻</a></li>
                <li><a href="#">商家后台</a></li>
            </ul>
            <ul>
                <li><a href="#"><h3>关于我们</h3></a></li>
                <li><a href="#">关于达内</a></li>
                <li><a href="#">联系我们</a></li>
                <li>
                    <img src="../img/footer/wechat.png" alt=""/>
                    <img src="../img/footer/sinablog.png" alt=""/>
                </li>
            </ul>
        </div>
        <div class="service">
            <p>达内商城客户端</p>
            <img src="../img/footer/ios.png" class="lf">
            <img src="../img/footer/android.png" alt="" class="lf"/>
        </div>
        <div class="download">
            <img src="../img/footer/erweima.png">
        </div>
    </div>
</div>
<script src="../js/jquery-3.1.1.min.js"></script>
<script src="../js/index.js"></script>
<script src="../js/slide.js"></script>
<script>
    $('.ck-slide').ckSlide({
        autoPlay: true,//默认为不自动播放，需要时请以此设置
        dir: 'x',//默认效果淡隐淡出，x为水平移动，y 为垂直滚动
        interval:3000//默认间隔2000毫秒
    });
    
    $('#input').blur(function(){
        if(this.validity.valueMissing){
        }else if(this.validity.tooShort){
        }else {
          var data = $("#input").val();
          if(!data){
            return;
          }
          /*发起异步GET请求，询问服务器用户名是否已经存在*/
          $.ajax({
    		  type:'post',
    		  data:'title='+data,
    		  url:'../../SelectBookServlet',
    		  success:function(data){
    	          console.log('开始处理响应数据');
    	          window.location.href = "search.jsp?title="+data;
    		  }
    	  });
        }
      });
</script>
</body>
</html>
