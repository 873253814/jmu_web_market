...
<%@ page import="jmu_web.market.book.recommended.dao.impl.*"%>
<%@ page import="jmu_web.market.book.recommended.dao.*"%>
<%@ page import="jmu_web.market.book.recommended.*"%>
...
<head>
    ...
    <%
    	...
    	List<Book> bookList = dao.getBookByIsbn1(isbn);
    	RecommendedDAO dao1 = new RecommendedDAOImpl();
    	List<Recommended> bookList1 = dao1.getAll();
    %>
</head>
<body>
<!-- 页面顶部-->
...
<div id="shop_detail">
    ...
    <div id="preview" class="lf">
        <div id="mediumDiv">
            <img id="mImg" src="../img/goods/<%=isbn %>/detail1big.jpg"/>
        </div>
        <div id="icon_all">
            <ul id="icon_list">
                <li class="i1"><img src="../img/goods/<%=isbn %>/detail1.jpg" /></li>
                <li class="i1"><img src="../img/goods/<%=isbn %>/detail2.jpg" onerror="this.style.display='none'" /></li>
                <li class="i1"><img src="../img/goods/<%=isbn %>/detail3.jpg" onerror="this.style.display='none'" /></li>
                <li class="i1"><img src="../img/goods/<%=isbn %>/detail4.jpg" onerror="this.style.display='none'" /></li>
                <li class="li"><img src="../img/goods/<%=isbn %>/detail5.jpg" onerror="this.style.display='none'" /></li>
            </ul>
        </div>
    </div>
    ...
    <div class="right_detail lf">
        ...
        <!-- 价格部分-->
        <div class="price">
            <div id="pro_price"><b>价格：</b><span>￥<%=b.getPrice() %></span></div>
            <div class="promise">
                <b>服务承诺：</b>
                <span>*退货补运费</span>
                <span>*30天无忧退货</span>
        ...
        <p class="style" id="choose_color">
            <s class="color">出版社：</s>
            <span><%=b.getPress() %></span>
        </p>
        <!-- 出版日期 -->
        <p>
            <s>出版日期：</s>
            <span><%=b.getPublished() %></span>
        </p>
        <!-- 未选择规格，颜色时状态-->
    ...
    <div id="demo" style="width:1000px;overflow:hidden;">
        <div id="indemo" style="float: left;width: 200%;">
            <div id="demo1" style="float:left"><!-- 第一个宽度显示 -->
                <%for(Recommended r:bookList1){ %>
                 <div class="detail_1 lf">
                    <div class="detail_img1">
                        <a href="detail.jsp?isbn=<%=r.getProduct()%>"><img src="../img/goods/<%=r.getProduct() %>/index.jpg" border="0"></a>
                    </div>
                    <p><%=r.getTitle() %></p>
                </div>
                    <%} %>
    ...
    <div class="left lf">
        <div class="all">
            ...
            <div id="parm">
                ...
                <ul>
                    <li><a href="#">图书编号：<%=b.getIsbn() %></a></li>
                    <li>
                    	<a href="#">图书名称：<%=b.getTitle() %></a>
					</li>
                    <li>
                    	<a href="#">作者：<%=b.getAuthor() %></a>
		            </li>
                    <li><a href="#">价格：￥<%=b.getPrice() %></a></li>
                    <li><a href="#">出版社：<%=b.getPress() %></a></li>
                    <li><a href="#">版次：<%=b.getEdtion() %></a></li>
                    <li><a href="#">出版日期：<%=b.getPublished() %></a></li>
                    <li><a href="#">页数：<%=b.getPages() %></a></li>
                    <li>
                    	<a href="#">字数:</a>
                    </li>
                    <li><a href="#">包装：<%=b.getPackaging() %></a></li>
                    <li><a href="#">开本：<%=b.getFormat() %></a></li>
                    <li><a href="#">用纸：<%=b.getForm() %></a></li>
                </ul>
            </div>
...
<script>
    $(function () {
        ...
        $("#collect").click(function (){
           	...
           	$.ajax({
                type: "post",
                url: '../../AddCollectServlet',
                data: "userId="+sid+"&product="+isbn,//params,
        ...
        $("#buy_now").click(function (e) {
            ...
            var isbn = $("#isbn").val();
            // 后台需要的参数
            var url = "order-confirm.jsp?product="+isbn+"&count="+num;
...
<script>
    //加入购物车操作
      ...
      $("#add_cart").click(function (e) {
        ...
        var isbn = $("#isbn").val();
        var sid = $("#sid").val();
/*         var url = "../../AddCartServlet?product="+isbn+"&count="+num+"&sid="+sid;
        window.location.href = url;
      	console.log("add_cart"); */
       	$.ajax({
            type: "post",
            url: '../../AddCartServlet',
            data: "product="+isbn+"&count="+num+"&sid="+sid,//params,
            success: function (data) {
                if (data.trim() == 'true') {
                	alert("已加入购物车！");
