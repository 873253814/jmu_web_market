...
<head>
    ...
    <%
    	String total = request.getParameter("zj");
    	String sid = (String)session.getAttribute("sessionId");
    	String orderPart = sid.substring(sid.length()-3,sid.length());
    	SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
    	SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    	String nowDate = df.format(new Date());
    	String nowDate1 = df1.format(new Date());
    	String orderId = orderPart + nowDate;
...
<!--订单确认-->
<form action="../../AddOrderServlet?nowDate=<%=nowDate1 %>&sid=<%=sid %>&rid=<%=rid %>&total=<%=total %>&count=<%=count %>&product=<%=product %>" method="post" id="pay_form">
    <div id="container" class="clearfix" >
        <!-- 支付订单信息-->
        <div class="pay_info">
            <b>支付金额：<i><%=total %>元</i></b><input type="hidden" name="payment" value=<%=total %>>
            <span>支付订单：<%=orderId %> 收款方：达内商城</span><input type="hidden" name="orderId" value=<%=orderId %>>
        </div>
        <!--支付方式-->
        <div id="pay_type">
            <!-- 支付方式头-->
